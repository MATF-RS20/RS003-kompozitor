...
public:
    ...
    void play(int index, Track* track) const;
    void stop(int index);

private:
    PlayManager() = default;
    std::map<int, sf::Sound> sound_data;
    ...
    float pitch_to_frequency(int pitch) const;
    void createKeyboardSound(int index, NoteTrack *note_track) const;
};
